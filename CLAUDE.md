...
1. **Expand food library for high-calorie/high-carb targets** — muscle-gain 3600+ kcal plans cap at ~72% calorie fit; add calorie-dense healthy carbs (rice bowls, potatoes, oats variants, dried fruit, smoothies) and bigger anchor servings.
2. **Plan Match on dashboard overview** + per-item "portion tuned for you" hint (uses `serving_scale ≠ 1`).
3. **Recipes for newer foods** — verify every engine food id has a recipe entry; add missing ones (recipes keyed without `food-`).
4. **Weekly view** — 7-day rotating plan (daySeed already supports per-day variation) + grocery list generation.
5. **Progress-aware feedback loop** — use logged weight trend to auto-adjust calorie target (e.g. ±100 kcal if losing too fast/slow).
6. **Alternatives portion-scaling** — swap options currently shown at 1× serving; scale them to slot context on swap.
7. **Medication-aware rules** — medications collected in onboarding but unused by engine (e.g. warfarin → vitamin-K consistency note, levothyroxine → breakfast timing note).
8. **Unit tests in repo** — move the ad-hoc sweep into `frontend/src/lib/__tests__/` with vitest so CI can run it.
9. **Cross-device sync** — would need real backend/login; localStorage is single-device (documented on Progress page).
10. **EmailJS setup** — user still needs to add the 3 env vars in Vercel for email sending to go live.
