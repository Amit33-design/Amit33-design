...
function buildEmailHtml(opts: {
  ...
  const statusColour = (st: string) =>
    st === "low" ? "#c2410c" : st === "over" ? "#dc2626" : "#15803d";
    ...
    <div style="padding:0 32px 24px">
      ...
      <div style="display:flex;gap:10px;margin-top:10px">
        ${clinical.glycemicLoad ? `<div style="flex:1;background:#f0f9ff;border-radius:12px;padding:12px 14px">
          ...
          <div style="font-size:18px;font-weight:800;color:#0369a1;text-transform:capitalize">${clinical.glycemicLoad.value} · ${clinical.glycemicLoad.band}</div>
        </div>` : ""}
        ${clinical.naK ? `<div style="flex:1;background:#f0fdf4;border-radius:12px;padding:12px 14px">
          ...
          <div style="font-size:18px;font-weight:800;color:#15803d">${clinical.naK.ratio} : 1</div>
        </div>` : ""}
      </div>` : ""}
...
<body style="margin:0;padding:0;background:#f8fafc;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif">
  <div style="max-width:640px;margin:32px auto;background:white;border-radius:20px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08)">
    ...
    <!-- Header -->
    <div style="background:linear-gradient(135deg,#6d28d9 0%,#0369a1 100%);padding:36px 32px;text-align:center">
    ...
    <div style="padding:0 32px 24px">
      ...
      <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:10px;text-align:center">
        <div style="background:linear-gradient(135deg,#fff7ed,#ffedd5);border-radius:14px;padding:16px 8px">
          <div style="font-size:22px;font-weight:800;color:#c2410c">${cal}</div>
        ...
        <div style="background:linear-gradient(135deg,#f0f9ff,#e0f2fe);border-radius:14px;padding:16px 8px">
          <div style="font-size:22px;font-weight:800;color:#0369a1">${carbs}g</div>
        ...
        <div style="background:linear-gradient(135deg,#f0fdf4,#dcfce7);border-radius:14px;padding:16px 8px">
          <div style="font-size:22px;font-weight:800;color:#15803d">${fat}g</div>
...
export default function ReportPage() {
  ...
  return (
    <div className="p-4 md:p-8 max-w-4xl mx-auto space-y-6 animate-fade-in">
        ...
        <div className="bg-gradient-to-br from-violet-50 to-sky-50 border border-violet-200 rounded-2xl p-5">
          ...
          <div className="flex gap-3 flex-wrap">
            ...
            <button
              ...
              className="px-5 py-2.5 rounded-xl bg-gradient-to-r from-violet-700 to-sky-700 text-white font-bold text-sm hover:opacity-90 disabled:opacity-40 disabled:cursor-not-allowed flex items-center gap-2"
            >
              {sending ? (
          ...
          {emailStatus === "sent"      && <p className="mt-3 text-sm text-emerald-700 font-semibold">✓ Report sent to {email}!</p>}
      ...
      >
        {/* Header gradient */}
        <div style={{ background: "linear-gradient(135deg, #6d28d9 0%, #0369a1 100%)" }} className="px-8 py-10 text-center text-white">
          <div className="text-5xl mb-3">💚</div>
          <h1 className="text-3xl font-black mb-2 tracking-tight">Your Health Report</h1>
          <p className="text-white/90 text-sm">Generated by healthCopilot &nbsp;·&nbsp; {date}</p>
        </div>
        ...
        <div className="p-8 space-y-8">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
              {[
                ...
                <div key={f.label} style={{ background: "#f8fafc" }} className="rounded-2xl p-4">
                  <div className="text-xs font-bold text-slate-600 uppercase tracking-wide">{f.label}</div>
          ...
          <section>
            ...
            <div className="grid grid-cols-4 gap-3 text-center">
              {[
                { v: `${cal}`,      u: "kcal",    bg: "#fff7ed", c: "#c2410c" },
                { v: `${protein}g`, u: "Protein",  bg: "#f5f3ff", c: "#7c3aed" },
                { v: `${carbs}g`,   u: "Carbs",    bg: "#f0f9ff", c: "#0369a1" },
                { v: `${fat}g`,     u: "Fat",      bg: "#f0fdf4", c: "#15803d" },
              ].map((m) => (
                <div key={m.u} className="rounded-2xl py-5 px-2" style={{ background: m.bg }}>
                  ...
                  <div className="text-xs font-bold mt-1" style={{ color: m.c }}>{m.u}</div>
                </div>
              ))}
            </div>
          </section>
                ...
                <div key={m.slot} className="flex gap-4 p-4 rounded-2xl" style={{ background: "#f8fafc" }}>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-baseline gap-2 mb-0.5">
                      ...
                      <span className="text-xs text-slate-600">{Math.round(m.slot_calories)} kcal · {Math.round(m.slot_protein_g)}g protein</span>
                    </div>
            ...
            <section>
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-600">Targets set from age, sex, diet and medical conditions</p>
                </div>
              </div>
                    ...
                    {clinical.nutrients.map((n) => (
                      <tr key={n.key} className="border-t border-slate-100 first:border-t-0">
                        <td className="px-4 py-2 text-slate-700">
                          {n.label}
                          {n.isLimit && <span className="text-slate-600 text-xs"> (limit)</span>}
                        </td>
                        ...
                        <td className={`px-4 py-2 text-right text-xs font-semibold whitespace-nowrap ${
                          n.status === "low" ? "text-orange-700" : n.status === "over" ? "text-red-600" : "text-emerald-700"
                        }`}>
                  ...
                  <thead>
                    <tr style={{ background: "#f1f5f9" }}>
                      {["Day", "🌅 Breakfast", "☀️ Lunch", "🌙 Dinner"].map((h) => (
                        <th key={h} className="px-3 py-2.5 text-left font-bold text-slate-600 uppercase tracking-wide">{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
          ...
          <section>
            ...
            {recentLogs.length > 0 ? (
              ...
              <div className="py-8 text-center text-slate-600 rounded-2xl" style={{ background: "#f8fafc" }}>
